...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		switch(easterEgg.toUpperCase())
		{
			...
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
				if (titleJSON.character != null && titleJSON.character.length > 0 && titleJSON.character != "none"){
			gfDance.frames = Paths.getSparrowAtlas(titleJSON.character);
		}else{
                gfDance.frames = Paths.getSparrowAtlas('gfDanceTitle');
                }
                if (titleJSON.animation != null && titleJSON.animation.length > 0 && titleJSON.animation != "none"){
		gfDance.animation.addByPrefix('danceLeft', titleJSON.animation, 24, false);
		gfDance.animation.addByPrefix('danceRight', titleJSON.animation, 24, false);	
		}else{
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['Kfir Engine by'], 15);
					#else
